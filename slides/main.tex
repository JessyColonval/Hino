...
\begin{frame}{Motivation}
	\textbf{What is an outlier?}
	\begin{itemize}
		\item This is a point that is inconsistent with the rest of the dataset.
	\end{itemize}
	\vspace{0.25cm}

	\textbf{Inconsistence have two effects on ML, in:}
	\begin{itemize}
		\item Learning phase: Weaken the predictive power.
		\item Validation phase: Weaken the cross-validation score.
	\end{itemize}
	\tikzarrow{0.75cm}{0.15cm}{0.1cm}They must be removed from these corpus.
	\vspace{0.5cm}
...
\begin{frame}{Hino's algorithm (based on \textbf{IQR})}
	\textbf{Isolation tolerance:}
	\begin{itemize}
		\item Being isolated $1$ time $\mathbf{\centernot\implies}$ being an outlier;
		\item A point can be isolated in several attributes.
	\end{itemize}
	\tikzarrow{0.75cm}{0.15cm}{0.1cm}Set a limit: $\mathbf{\lfloor ( 0.0205 * log_{2}(-1.623730 + n_{c}) + 0.062579 ) * n_{a} \rceil}$
	\vspace{0.25cm}

	\textbf{How it works:}
	\begin{itemize}
		\item Splits points into several quantiles for each attribute;
		\item \textbf{Isolation score of a point}: number of times where its class isn't present in the $2$ adjacent quantiles;
		\item \textbf{Outlier}: isolation score $\geq$ tolerance limit;
		\item \textbf{Restarts with tolerance limit} $\mathbf{+1}$ if:
			\begin{itemize}
				\item \% outlier detected $>$ the \% allowed.
				\item a class is removed.
			\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}{Experimentation objectives}
	...
	\item A \textcolor{ForestGreen}{\textbf{viable}} method:
		...
		\end{itemize}
	\item \textcolor{ForestGreen}{\textbf{Better}} than the state-of-the-art methods:\\ IQR [11], SVM [2], Isolation Forest [7] and LOF [4].
	\end{itemize}
	\vspace{0.25cm}
...
\begin{frame}{Synthetic datasets}
	...
	\begin{itemize}
	\item Generate with the algorithm used to create Madelon (from UCI) [6].
	\item Modify $5\%$ of points and make them outliers:
	\begin{itemize}
		\item Preserve class representativity.
...
\begin{frame}{Results on synthetic datasets}
...
\begin{frame}{Conclusion \& perspective}
	...
	\item \textcolor{BrickRed}{\textbf{Takes more time than IQR}}: $O(n_{p}(n_{a}n_{c}+n_{a}+1))$ vs $O(n_{p}*2n_{a})$\\
	...
	\begin{itemize}
	\item Use a trained \textbf{Hino} model to detect outliers.
...
\begin{frame}{Bibliography}
	\scriptsize
	\textbf{[2]} Amer, M., Goldstein, M., Abdennadher, S.: Enhancing one-class support vector machines for unsupervised anomaly detection. In: Proceedings of the ACM SIGKDD Workshop on Outlier Detection and Description. pp. 8–15. ODD’13, Association for Computing Machinery, New York, NY, USA (Aug 2013). \url{https://doi.org/10.1145/2500853.2500857}
	\vspace{0.25cm}

	\textbf{[4]} Breunig, M., Kriegel, H.P., Ng, R., Sander, J.: LOF: Identifying Density-Based Local Outliers. vol. 29, pp. 93–104 (Jun 2000). \url{https://doi.org/10.1145/342009.335388}
	\vspace{0.25cm}

	\textbf{[6]} Guyon, I.: Design of experiments for the NIPS 2003 variable selection benchmark. In: undefined (2003). \url{https://doi.org/10.1007/978-3-540-35488-8_10}
	...
	\textbf{[7]} Liu, F.T., Ting, K.M., Zhou, Z.H.: Isolation-Based Anomaly Detection. ACM Transactions on Knowledge Discovery from Data 6(1), 3:1–3:39 (Mar 2012). \url{https://doi.org/10.1145/2133360.2133363}
	\vspace{0.25cm}

	\textbf{[11]} Vinutha, H.P., Poornima, B., Sagar, B.M.: Detection of Outliers Using Interquartile Range Technique from Intrusion Dataset. In: Satapathy, S.C., Tavares, J.M.R., Bhateja, V., Mohanty, J.R. (eds.) Information and Decision Sciences. pp. 511–518. Springer, Singapore (2018). \url{https://doi.org/10.1007/978-981-10-7563-6_53}
\end{frame}

\begin{frame}{Bonus: IQR algorithm}
	\begin{minipage}[c]{0.6\linewidth}
