...
\begin{frame}{Hino's algorithm}
	...
	\begin{itemize}
		...
		\item \textbf{Restarts with tolerance limit} $\mathbf{+1}$ if \% outlier detected $>$ the \% allowed.
	\end{itemize}
\end{frame}
...
\begin{frame}{Synthetic datasets}
	...
	\begin{itemize}
		\item Preserve representativity of the dataset.
...
\begin{frame}{Conclusion \& perspective}
	...
	\item Takes more time than IQR: $O(n_{p}(n_{a}n_{c}+n_{a}+1))$ vs $O(n_{p}*2n_{a})$\\
